...
void OBCameraNode::publishDepthPointCloud(const std::shared_ptr<ob::FrameSet> &frame_set) {
  ...
  for (size_t i = 0; i < point_size; i++) {
    bool valid_point = points[i].z >= min_depth && points[i].z <= max_depth; //In mm, Added "&& points[i].y <= 260" (Lower number == further away from floor)
...
void OBCameraNode::publishColoredPointCloud(const std::shared_ptr<ob::FrameSet> &frame_set) {
  ...
  for (size_t i = 0; i < color_width * color_height; i++) {
    bool valid_point = point_cloud[i].z >= min_depth && point_cloud[i].z <= max_depth && point_cloud[i].y <= 220; //In 28cm ,Added "&& points[i].y <= 280.0" to remove flooring *Note* on robot 220, on testing 280
    if (valid_point || ordered_pc_) {
      *iter_x = static_cast<float>(point_cloud[i].x / 1000.0);// Converting to meters
